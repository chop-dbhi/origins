...
func (b *Buffer) Write(buf Facts) (int, error) {
	if len(buf) == 0 {
		return 0, nil
	}
...
// WriteFact writes a single fact to the buffer.
func (b *Buffer) WriteFact(f *Fact) error {
	_, err := b.Write(Facts{f})
	return err
}
...
// NewBuffer returns a buffer of facts.
